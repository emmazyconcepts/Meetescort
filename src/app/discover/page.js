...
export default function DiscoverPage() {
  ...
  const loadProfilesAndAds = async () => {
    ...
    try {
      ...
      const filteredProfiles = filterProfilesByLocation(usersData, searchLocation);
  ...
  const combinedResults = useMemo(() => {
    const profilesWithAds = profiles.map(profile => {
      ...
      return profileAd ? { ...profile, isAd: true, adData: profileAd } : profile;
    });
    ...
    return profilesWithAds.sort((a, b) => {
      ...
      switch (filters.sortBy) {
        case 'recent':
          return new Date(b.lastUpdated?.toDate?.() || b.lastUpdated) - 
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-white mb-4">
            {searchLocation ? `Profiles in ${searchLocation}` : 'Discover Profiles'}
          </h1>
        ...
        <div className="bg-black/30 rounded-2xl p-6 mb-8 border border-pink-500/20">
          <form onSubmit={(e) => {
            ...
            if (location) {
              ...
              window.history.pushState({}, '', `/discover?location=${encodeURIComponent(location)}`);
            }
          }}>
        ...
        <div className="bg-black/30 rounded-2xl p-6 mb-8 border border-pink-500/20">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <button 
              onClick={() => setFilters({ gender: '', ageRange: '', services: [], sortBy: 'recent' })}
        ...
        {loading ? (
          ...
          <>
            ...
            {searchLocation && !hasExactMatches && locationHierarchy.length > 1 && (
              <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-4 mb-6">
                <p className="text-yellow-200">
                  No exact matches found for <strong>{searchLocation}</strong>. 
                  Showing results for <strong>{locationHierarchy[1]}</strong> and surrounding areas.
                </p>
              </div>
            )}
            ...
            <div className="flex justify-between items-center mb-6">
              <p className="text-pink-200">
                Found {combinedResults.length} {combinedResults.length === 1 ? 'profile' : 'profiles'}
            ...
            {combinedResults.length === 0 ? (
              <div className="text-center py-16 bg-black/20 rounded-2xl border border-pink-500/20">
                ...
                <p className="text-pink-200 mb-4">Try searching for a different location or broader area.</p>
                <button 
                  onClick={() => {
                    setSearchLocation('');
