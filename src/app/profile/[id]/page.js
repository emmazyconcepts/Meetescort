    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 to-black flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-pink-200 mb-6">{error || 'The profile you are looking for does not exist.'}</p>
          <Link href="/discover" className="bg-pink-500 text-white px-6 py-3 rounded-lg hover:bg-pink-600 transition duration-300">
      ...
      <nav className="bg-black/30 backdrop-blur-lg border-b border-pink-500/20 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <Link href="/discover" className="text-pink-300 hover:text-white transition duration-300">
              ← Back to Discover
            </Link>
            ...
            <div className="bg-black/30 rounded-2xl border border-pink-500/20 overflow-hidden">
              ...
              <div className="border-b border-pink-500/20">
                <nav className="flex space-x-8 px-6">
                  {['photos', 'about', 'services', 'availability'].map((tab) => (
                    <button
                      key={tab}
              ...
              {/* Tab Content */}
              <div className="p-6">
                {activeTab === 'photos' && <PhotoGallery profile={profile} onImageSelect={setSelectedImage} />}
                {activeTab === 'about' && <AboutSection profile={profile} />}
                {activeTab === 'services' && <ServicesSection profile={profile} />}
    ...
    <div className="bg-black/30 rounded-2xl border border-pink-500/20 p-6 sticky top-24">
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="text-center p-3 bg-black/20 rounded-lg">
          <div className="text-white font-bold text-lg">{profile.views || 0}</div>
          <div className="text-pink-200 text-sm">Profile Views</div>
        </div>
        <div className="text-center p-3 bg-black/20 rounded-lg">
          <div className="text-white font-bold text-lg">{profile.photos?.length || 0}</div>
      ...
      <div className="space-y-3">
        ...
        {profile.incallPrice && (
          <div className="flex justify-between items-center p-2">
            ...
            <span className="text-green-400 font-semibold">${profile.incallPrice}</span>
          </div>
        )}
        ...
        {profile.outcallPrice && (
          <div className="flex justify-between items-center p-2">
            ...
            <span className="text-green-400 font-semibold">${profile.outcallPrice}</span>
          </div>
        )}

        {profile.website && (
          <a href={profile.website} target="_blank" rel="noopener noreferrer" className="block text-center bg-blue-500/20 text-blue-300 py-2 rounded-lg hover:bg-blue-500/30 transition duration-300">
            Visit Website
          </a>
        )}

        <button className="w-full bg-gradient-to-r from-pink-500 to-purple-600 text-white py-3 rounded-lg font-semibold hover:from-pink-600 hover:to-purple-700 transition duration-300 mt-4">
          💌 Send Message
        </button>
      </div>
    </div>
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          <h4 className="text-lg font-semibold text-pink-300">Basic Information</h4>
        ...
        <div className="space-y-4">
          ...
          {profile.website && (
            <div>
              ...
              <a href={profile.website} target="_blank" rel="noopener noreferrer" className="text-blue-300 hover:text-blue-400 ml-2">
                {profile.website}
              </a>
            </div>
          )}
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-3">
            {profile.incallPrice && (
              <div className="flex justify-between items-center p-3 bg-black/30 rounded-lg">
                ...
                <span className="text-green-400 font-bold text-lg">${profile.incallPrice}</span>
              </div>
            )}
            {profile.outcallPrice && (
              <div className="flex justify-between items-center p-3 bg-black/30 rounded-lg">
                <span className="text-pink-200">Outcall Service</span>
                <span className="text-green-400 font-bold text-lg">${profile.outcallPrice}</span>
              </div>
            )}
          </div>
        </div>
        ...
        {/* Service Details */}
        <div className="bg-black/20 rounded-xl p-6 border border-pink-500/20">
          <h4 className="text-lg font-semibold text-pink-300 mb-4">Service Information</h4>
          <div className="space-y-3">
            <InfoRow label="Caters To" value={profile.catersTo} />
            ...
            {/* Service Tags */}
            <div>
              <span className="text-pink-200 font-medium">Services Offered:</span>
              <div className="flex flex-wrap gap-2 mt-2">
                {profile.incallPrice && <span className="bg-pink-500/20 text-pink-300 px-3 py-1 rounded-full text-sm">Incall</span>}
                {profile.outcallPrice && <span className="bg-purple-500/20 text-purple-300 px-3 py-1 rounded-full text-sm">Outcall</span>}
                <span className="bg-blue-500/20 text-blue-300 px-3 py-1 rounded-full text-sm">Professional</span>
                <span className="bg-green-500/20 text-green-300 px-3 py-1 rounded-full text-sm">Verified</span>
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Weekly Schedule */}
        <div className="bg-black/20 rounded-xl p-6 border border-pink-500/20">
          <h4 className="text-lg font-semibold text-pink-300 mb-4">Weekly Schedule</h4>
          <div className="space-y-3">
            {days.map(day => {
              const availability = profile.availability?.[day];
              return (
                <div key={day} className="flex justify-between items-center p-3 bg-black/30 rounded-lg">
                  ...
                  <div className="text-right">
                    <span className={`px-2 py-1 rounded text-sm ${
                      ...
                      {availability?.available ? availability.hours || 'Available' : 'Not Available'}
                    </span>
                  </div>
                </div>
              );
        ...
        {/* Booking Information */}
        <div className="bg-black/20 rounded-xl p-6 border border-pink-500/20">
          <h4 className="text-lg font-semibold text-pink-300 mb-4">Booking Information</h4>
          <div className="space-y-4">
            <div>
              <h5 className="text-pink-200 font-medium mb-2">📞 Contact Methods</h5>
              <div className="space-y-2">
                <button className="w-full bg-green-500/20 text-green-300 py-2 rounded-lg hover:bg-green-500/30 transition duration-300">
                  Send Message
                </button>
                <button className="w-full bg-blue-500/20 text-blue-300 py-2 rounded-lg hover:bg-blue-500/30 transition duration-300">
                  Request Booking
                </button>
              </div>
            </div>
            ...
            <div>
              <h5 className="text-pink-200 font-medium mb-2">ℹ️ Important Notes</h5>
