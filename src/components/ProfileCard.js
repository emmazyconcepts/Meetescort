...
export default function ProfileCard({ profile }) {
  ...
  const incallPrice = profile.incallPrice ? `$${profile.incallPrice}` : 'Contact for pricing';
      ...
      <div className="bg-black/30 rounded-2xl overflow-hidden border border-pink-500/20 hover:border-pink-500/40 transition-all duration-300 group-hover:scale-105 group-hover:shadow-2xl group-hover:shadow-pink-500/20">
        ...
        <div className="p-4">
          <div className="flex justify-between items-start mb-2">
            <h3 className="font-bold text-white text-lg truncate">{profile.displayName}</h3>
          ...
          <div className="flex flex-wrap gap-1 mb-3">
            {profile.incallPrice && (
              <span className="bg-blue-500/20 text-blue-300 px-2 py-1 rounded text-xs">Incall</span>
            )}
            {profile.outcallPrice && (
              <span className="bg-purple-500/20 text-purple-300 px-2 py-1 rounded text-xs">Outcall</span>
            )}
          </div>
